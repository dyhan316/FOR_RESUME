...
def run_experiment(trial, kf_split, label_tv, label_test):  
    for FOLD, (train_idx, valid_idx) in enumerate(kf_split):
        ...
        wandb.init(project=config.wandb_name, config=config, name=f"fold{FOLD}", job_type=job_type, reinit=True,
                   group = f"trial_{trial.number}_{config.task}-{config.layer_control}_{str(config.pretrained_path).split('/')[-1].split('.')[0]} \
                             _tr_{config.train_num}_batch_{config.batch_size}}_sched_{config.lr_schedule}_strat_{config.stratify}") #becasue optuna,we have to redo the trials
