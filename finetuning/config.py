...
kisti_data = {"ABCD": '/scratch/x2519a05/data/1.ABCD/T1_MNI_resize128', # should be implemented for multimodal data i.e. FA
              "CHA": '/scratch/x2519a05/data/CHA_bigdata/sMRI_brain',
              "CHA_raw": '/scratch/x2519a05/data/CHA_bigdata/sMRI_raw',
              "CHA_resize128": '/scratch/x2519a05/data/CHA_bigdata/sMRI_resize128'}
kisti_csv = {"ABCD": '/scratch/x2519a05/data/1.ABCD/4.demo_qc/ABCD_phenotype_total_balanced_multitarget.csv',
             "CHA": '/scratch/x2519a05/data/CHA_bigdata/metadata/sMRI_BSID_imputed.csv'}
...
class Config:
    ...
    def _set_default_config(self, task):
        ...
        elif self.input_option == "resize128":
            self.resize_method = None
    ...
    def _set_task_config(self, task):
        self.data = self.data_path[self.dataset] if self.data_type == None else self.data_path[self.dataset+'_'+self.data_type]
    ...
    def _set_CHA_config(self, task):
        ...
        self.psm_label = ['age(days)', 'BSID-II_Mental_score', 'BSID-II_Motor_score']
        self.iter_strat_label = {"binary" : ['sex'], "multiclass" : [] ,"continuous" : ["age(days)"]}
        if "ASDGDD" in task : 
            self.task_type = 'cls'
            self.num_classes = 1 # since we use BCEwithLogit loss
