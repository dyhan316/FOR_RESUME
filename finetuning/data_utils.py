...
from utils import get_info_fold
...
def get_folds_original(config):
    ...
    images = pd.DataFrame({'files': glob(config.data+"/*[yz]")}) # for .npy or .nii.gz
    images[config.subjectkey] = images.files.map(lambda x: x.split('/')[-1].split(".")[0])
    labels = labels[labels[config.subjectkey].isin(images[config.subjectkey])]
    ...
    if config.task_type == 'cls':
        ...
        if config.stratify in ["iter_strat", "balan_iter_strat"]: 
        ...
        if 'balan' in config.stratify: # - jubin modified
            limit = min(len(data_1), len(data_2))
            data_1 = data_1.sample(n=limit)
            data_2 = data_2.sample(n=limit)
        ...
        config.train_num = int((len(data_1_rem)+len(data_2_rem))*(1-config.valid_ratio)) if config.train_num == 'all' else config.train_num
        ...
        if not config.eval and (config.train_num*(1+config.valid_ratio) > (len(data_1) + len(data_2) - len_1_test - len_2_test)):
            warnings.warn('Not enough valid data. Set smaller --train_num or smaller config.valid_ratio in config.py.')
        ...
        if "test" not in config.task and (len(label_test) < 50): #i.e. if we're not running test version and actually running:
            warnings.warn(f'Not enough test data. (Total: {len(label_test)})')
    ...
    SPLITS = config.num_folds #CHANGE TO 5!!!
    ...
    if config.stratify == 'strat'or config.stratify == "balan_strat": 
        kf = StratifiedKFold(n_splits = SPLITS) 
        skf_target =  [label_tv, label_tv[label_name]] 
        get_info_fold(kf.split(*skf_target), label_tv, cols_to_see)
    elif config.stratify == 'iter_strat' or config.stratify == "balan_iter_strat" : 
        kf = IterativeStratification(n_splits= SPLITS, order=10, random_state = np.random.seed(0))#0) #np.random.RandomState(0) #increasing order makes it similar if config.stratify == "iter_strat" 
        ...
        raise NotImplementedError(f"{config.stratify} is not implemented!")
    ###ACTUALLY RUNNING CROSS-VALIDATION/TRAINING/EVALUATION (depending on mode)        
    return [*kf.split(*skf_target)], label_tv, label_test


def run_psm(labels, treatment, config):
    df_psm = labels.copy().loc[:,[config.subjectkey, treatment]+config.psm_label]
    psm = PsmPy(df_psm, treatment=treatment, indx=config.subjectkey)
...
def get_psm_balan_iter_strat_folds(config):
    '''This option presupposes that the task is binary classification.'''
    label_name = config.label_name
    subjectkey = config.subjectkey
    labels = pd.read_csv(config.label)

    # get image files to filter subjects that have MRI images
    images = pd.DataFrame({'files': glob(config.data+"/*[yz]")}) # for .npy or .nii.gz
    images.loc[:,subjectkey] = images.files.map(lambda x: x.split('/')[-1].split(".")[0])
    labels = labels[labels.subjectkey.isin(images.subjectkey)]

    # get subjects that have a target label
    task_include = config.task_name.split('/')
    config.task_include = task_include # needed later

    # divide each group according to the task & check which one is larger.
    labels = labels[labels[label_name].isin(task_include)].reset_index(drop=True)
    counts = labels[label_name].value_counts()
    minor, major = counts.index[counts.argmin()], counts.index[counts.argmax()]
    if major in labels.columns:
        treatment = major
    elif minor in labels.columns:
        treatment = minor
    else:
        raise ValueError("Inappropriate label to run propensity score matching")

    # run propensity score matching & select one group which have matched_ID
    df_matched = run_psm(labels=labels, treatment=treatment, config=config)
    df_matched = df_matched[[subjectkey,'matched_ID']].dropna()
    label_kfold = labels[labels.subjectkey.isin(df_matched.subjectkey)]
    
    # do Iterative stratification
    binary_cols = list(set(config.iter_strat_label['binary'] + [label_name])) #add the label_name if it is new
    floatized_arr = multilabel_matrix_maker(label_kfold, n_chunks=10,
                                            binary_cols=binary_cols, 
                                            multiclass_cols=config.iter_strat_label['multiclass'],
                                            continuous_cols=config.iter_strat_label['continuous'])
    skf_target = [floatized_arr, floatized_arr]        
    kf = IterativeStratification(n_splits= int(1/config.test_ratio), order=10, random_state = np.random.seed(0))
    folds = [*kf.split(*skf_target)]
    
    # after getting k-folds, separate it into test split & train/valid split
    real_folds=[] # extract indices for each fold
    for t_idx, v_idx in folds:
        psm_val_idx = label_kfold.iloc[v_idx].index # treatment group
        matched_valid_idx = labels[labels.subjectkey.isin(df_matched.iloc[v_idx].matched_ID)].index # matched group
        real_folds.append(psm_val_idx.append(matched_valid_idx))
        
    # make test split
    config.test_fold = 0
    label_test = labels.iloc[real_folds[config.test_fold]]
    real_folds.pop(config.test_fold)
    
    # make train/valid split
    tv_idx = []
    for i in range(len(real_folds)):
        folds_ = real_folds[:]
        val_idx = folds_.pop(i)
        tv_idx.append((np.concatenate(folds_), val_idx))
    ...
    cols_to_see.append(config.label_name)
    cols_to_see = list(set(cols_to_see)) #remove redundancy
    get_info_fold(tv_idx, labels, cols_to_see)
    
    # change labels into 0,1
    labels[label_name].replace({task_include[0]: 0, task_include[1]: 1}, inplace=True)
    label_test[label_name].replace({task_include[0]: 0, task_include[1]: 1}, inplace=True)
    
    return tv_idx, labels, label_test
...
def get_images(labels, config):   
    total_subj_imgs = pd.DataFrame({'files': glob(config.data+"/*[yz]")})
    total_subj_imgs[config.subjectkey] = total_subj_imgs.files.map(lambda x: x.split('/')[-1].split(".")[0])

    images_split = defaultdict()
    for split in ['train', 'valid', 'test']:
        images_split[split] = pd.merge(total_subj_imgs, labels[split], on=config.subjectkey, how='inner')
