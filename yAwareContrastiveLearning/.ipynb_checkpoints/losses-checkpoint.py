...
def multi_rbf_kernel(y1,y2,gamma):
    """
    creates a mulit-dim versino of the yAware contrastive loss, taking into account different sigma values for different albels.
    y1 : shape 2N, * (* : feautre size (number of labels)
    y2 : shape 2N, * (* : feautre size (number of labels)
    gamma : shape * (i.e. either single float or multiple gamma    
    """
    rbf_total = 1
    for i,gamma_i in enumerate(gamma) :  #for each dimension 
        y1_i = y1[:,i].reshape(-1,1)
        y2_i = y2[:,i].reshape(-1,1) #2Nx1
        rbf_i = rbf_kernel(y1_i, y2_i, gamma = gamma_i)#[2Nx2N]
        rbf_total = rbf_total*rbf_i
    return rbf_total
...
class GeneralizedSupervisedNTXenLoss(nn.Module):
    def __init__(self, config, kernel='rbf', temperature=0.1, return_logits=False, sigma=1.0):
        ...
        self.sigma = sigma
        if self.kernel == 'rbf':
            #self.kernel = lambda y1, y2: rbf_kernel(y1, y2, gamma=1./(2*self.sigma**2)) #single dim version 
            self.kernel = lambda y1, y2: multi_rbf_kernel(y1, y2, gamma=[1./(2*sigma**2) for sigma in self.sigma])
