...
from yAwareContrastiveLearning_multilabel import yAwareCLModel
...
#def main(trial) :  #only activate when trying to implement optuna
    ...
    if config.mode == 'pretraining':        
        ...
        if args.dataset2use in ["ABCD"] : 
            file_type = "npy"
            
        else : 
            file_type = "nii"
            
        dataset_train = UKBDataset(config, subj_train, file_type = file_type) #MRIDataset(config, training=True)
        dataset_val = UKBDataset(config,subj_val, file_type = file_type)  #MRIDataset(config, validation=True)
    else:
